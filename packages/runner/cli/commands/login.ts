...
export const loginCommand = new Command('login')
  ...
  .action(async (opts: { cloudUrl: string; email?: string; password?: string }) => {
    ...
    // Save to local config. Loading with no existing config returns
    // an empty object — the schema accepts that and the user fills
    // credentials in via `coro init` or the dashboard.
    ...
    console.log('  1. Run \x1b[36mcoro init\x1b[0m to configure your Anthropic API key and git credentials')
    console.log('  2. Run \x1b[36mcoro runner start\x1b[0m to start the runner')
  })
