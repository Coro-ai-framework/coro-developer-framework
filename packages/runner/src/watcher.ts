...
function categorise(filePath: string, coroIntelligenceDir: string): ChangeCategory {
  const rel = path.relative(coroIntelligenceDir, filePath)
...
/**
 * Watches the Coro intelligence repo for changes to:
 ...
 * The Agent Host pulls the latest intelligence repo at the start of each Claude turn, so
...
export function startWatcher(ctx: WatcherContext): FSWatcher {
  ...
  const watched = [
    path.join(coroIntelligenceDir, 'memory'),
    path.join(coroIntelligenceDir, 'agents'),
    path.join(coroIntelligenceDir, 'workflows'),
    path.join(coroIntelligenceDir, '.claude'),
    path.join(coroIntelligenceDir, 'config'),
    path.join(coroIntelligenceDir, 'tools', 'src'),
  ]

  const ignored = [
    path.join(coroIntelligenceDir, '.git'),
    path.join(coroIntelligenceDir, 'tools', 'dist'),
    path.join(coroIntelligenceDir, 'tools', 'node_modules'),
    /node_modules/,
    /\.git/,
  ]
...
async function processChanges(changedFiles: string[], ctx: WatcherContext): Promise<void> {
  ...
  const status = await gitClient.getStatus(coroIntelligenceDir)
  if (status === 'nothing to commit, working tree clean') {
    logger.debug('No uncommitted changes in intelligence repo — skipping')
    return
  }
  ...
  const workflowMds = changedFiles.filter(f => {
    const rel = path.relative(coroIntelligenceDir, f)
  ...
  if (yamlFiles.length > 0) {
    const result = await validateYamlFiles(yamlFiles, coroIntelligenceDir, logger)
    if (!result.ok) {
      await writeValidationFailure(coroIntelligenceDir, 'yaml-parse', yamlFiles, result.detail, logger)
      return
    }
  }
  ...
  if (workflowMds.length > 0) {
    ...
    if (!result.ok) {
      await writeValidationFailure(coroIntelligenceDir, 'workflow-config', workflowMds, result.detail, logger)
      return
    }
  }
  ...
  if (skillFiles.length > 0) {
    ...
    if (!result.ok) {
      await writeValidationFailure(coroIntelligenceDir, 'skill-frontmatter', skillFiles, result.detail, logger)
      return
    }
  }

  if (tsFiles.length > 0) {
    const toolsDir = path.join(coroIntelligenceDir, 'tools')
    ...
    try {
      ...
      await writeValidationFailure(coroIntelligenceDir, 'typescript-build', tsFiles, output, logger)
      return
    }
  }

  // Build branch name and commit message from the changed files
  const categories = [...new Set(changedFiles.map(f => categorise(f, coroIntelligenceDir)))]
  const slug = buildSlug(changedFiles, coroIntelligenceDir)
  ...
  const commitMsg = buildCommitMessage(categories, mdFiles, tsFiles, coroIntelligenceDir)

  // Determine which branch we're currently on so we can return to it afterwards
  const g = simpleGit({ baseDir: coroIntelligenceDir })
  ...
  const g2 = simpleGit({ baseDir: coroIntelligenceDir })
  const allRemotes = await g2.getRemotes(false)
  const pushRemote = allRemotes.find(r => r.name === 'bitbucket') ? 'bitbucket' : 'origin'

  try {
    // Create branch, commit, push
    await gitClient.checkoutBranch(coroIntelligenceDir, branch, true)
    await gitClient.commitAll(coroIntelligenceDir, commitMsg)
    await gitClient.pushToRemote(coroIntelligenceDir, pushRemote, branch)
    ...
    try { await gitClient.checkoutBranch(coroIntelligenceDir, currentBranch) } catch { /* best effort */ }
    return
  }

  // Always return to the original branch so the repo isn't left on the improvement branch
  try {
    await gitClient.checkoutBranch(coroIntelligenceDir, currentBranch)
  ...
  const remoteUrl = await getRemoteRepoSlug(coroIntelligenceDir, logger)
  const repoSlug = remoteUrl ?? path.basename(coroIntelligenceDir)
  const prDescription = buildPrDescription(categories, changedFiles, coroIntelligenceDir)
  ...
  try {
    const job = await stateBackend.createJob({
      ...
      params: {
        ...
        repoSlug,
        branchName: branch,
        serviceName: 'coro-intelligence',
        changedFiles: changedFiles.map(f => path.relative(coroIntelligenceDir, f)),
      },
    })
...
function buildSlug(files: string[], coroIntelligenceDir: string): string {
  const dirs = files
    .map(f => path.relative(coroIntelligenceDir, path.dirname(f)))
...
function buildCommitMessage(
  ...
  if (mdFiles.length > 0) parts.push(`update ${[...new Set(mdFiles.map(f => categorise(f, coroIntelligenceDir)))].join(', ')} files`)
...
async function validateYamlFiles(
  ...
  for (const filePath of files) {
    try {
      ...
      errors.push(`${path.relative(coroIntelligenceDir, filePath)}: ${String(err)}`)
    }
  }
...
async function writeValidationFailure(
  ...
  try {
    const failurePath = path.join(
      coroIntelligenceDir, 'memory', 'proposals',
    ...
    const content = [
      ...
      `**Files:** ${files.map(f => path.relative(coroIntelligenceDir, f)).join(', ')}`,
      '',
      '## Error output',
      '',
      '```',
...
function buildPrDescription(
  ...
  for (const file of changedFiles) {
    const cat = categorise(file, coroIntelligenceDir)
    const rel = path.relative(coroIntelligenceDir, file)
  ...
  const lines = [
    ...
    '',
    'This PR was opened automatically by the Coro intelligence file watcher.',
    'An agent proposed these changes during a job run.',
    '',
    '> **Review carefully before merging.** Once merged, the runner pulls',
    '> the latest intelligence at the start of the next Claude turn — changes',
    '> take effect immediately for all subsequent jobs.',
    '',
    '## Changed files',
    '',
  ]
