...
export function buildContributionBriefing(findings: ReadonlyArray<OssContributionFinding>): string {
  const header = [
    ...
    'if none couple — and leave the rest out, named in the plan\'s Deferred',
    'section. Do not open a stack of PRs, and do not `escalate` a leftover:',
    'that ends the job and loses the PR the coupled set earned. The runner',
    'raises whatever never reached a PR once this job finishes.',
    '',
