...
/**
 * Propose one or more file changes to the Coro intelligence repo.
 *
...
export async function proposeChange(
  ...
  const coroIntelligenceDir = ctx.settings.paths.coroIntelligenceDir
  ...
  // Validate all paths are within coroIntelligenceDir
  for (const file of files) {
    const abs = path.resolve(coroIntelligenceDir, file.path)
    if (!abs.startsWith(coroIntelligenceDir + path.sep)) {
      throw new Error(`File path "${file.path}" escapes coroIntelligenceDir`)
    }
  }

  // 1. Write the human-readable proposal summary
  const proposalDir = path.join(coroIntelligenceDir, 'memory', 'proposals')
  ...
  const written: string[] = [path.relative(coroIntelligenceDir, proposalPath)]

  // 2. Write all proposed files
  for (const file of files) {
    const absPath = path.resolve(coroIntelligenceDir, file.path)
  ...
  return {
    proposalFile: path.relative(coroIntelligenceDir, proposalPath),
...
export async function listProposals(
  ...
  const proposalDir = path.join(ctx.settings.paths.coroIntelligenceDir, 'memory', 'proposals')
