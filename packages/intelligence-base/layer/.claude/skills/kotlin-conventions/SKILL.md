...
Run `gradle` and `git` from the repo checkout (`cd <repoCheckoutDir> && …`). See **golang-conventions** for Coro path layout; add Kotlin-specific build recipes here per tenant.
