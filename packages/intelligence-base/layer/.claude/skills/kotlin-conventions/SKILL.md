...
Run `gradle` from the repo checkout (`cd <repoCheckoutDir> && …`). For git, prefer `git -C <repoCheckoutDir> …` (chained `cd … && git …` only as fallback). See **golang-conventions** for Coro path layout; add Kotlin-specific build recipes here per tenant.
