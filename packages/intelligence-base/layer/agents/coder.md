...
1. **Hand off to the review/gatekeeper phase by ending your turn.** The runner auto-advances to the phase after `coding` in this workflow (see `job.workflowPhases` — e.g. `review` on the default lane, `review-and-verify` on the fast lane). Do NOT call `goto_phase("coding")` to start the next work item. That next phase opens the PR for the current work item (after any interactive preview approval), then gates and merges it before routing back to coding for the next item.
...
8. **Route control back explicitly with `goto_phase` — this is the step that prevents the phase from hanging.** Posting a summary/reply and then stopping is **not** a phase transition; without an explicit signal the coding phase can sit idle until the idle watchdog has to step in. Look up the phase immediately after `coding` in `job.workflowPhases` and call `goto_phase("<that-phase>")` as the last action of your turn:
   - **Gatekeeper loop-back** → `goto_phase` to the review/gatekeeper phase so the merge gatekeeper re-checks the updated PR and proceeds to approval/merge.
   - **Evaluator loop-back** → `goto_phase` to the review/gatekeeper phase so the merge gatekeeper opens/merges the fix PR; once it lands, the runner advances to the next workflow phase (e.g. `evaluation` on the default lane).
...
- **End every coding phase with an explicit control signal.** After pushing the branch + posting the preview for a **forward** handoff, **end your turn** — the runner auto-advances to the review/gatekeeper phase declared in `job.workflowPhases`. For **backward** loops (fix after review feedback), call `goto_phase` targeting that same review/gatekeeper phase. Use `goto_phase("coding")` only when you are already in the review phase and need to re-enter coding for a fix. Park with `await_event(...)`, or stop with `escalate(...)` when blocked. There is no "complete this phase" tool. If you need additional developer input mid-phase, call `await_event({ eventName: "developer-input: <reason>" })`.
- **One work item per coding phase.** When you finish the current work item (branch pushed + preview posted), end your turn so the merge gatekeeper phase can open and merge the PR(s), then hand control back to you for the next work item. Calling `goto_phase("coding")` to start the next work item without going through the gatekeeper skips review for the work item you just finished.
- **Heed `[coding-preflight]` and "Open PRs on this job".** The runner prepends a preflight warning when you enter `coding` while the current work item already has open PRs (i.e. the review phase already opened one), and every phase kickoff lists all open mappings. If you see open PRs for the current WI and you are not in a fix loop, end your turn to advance to the review/gatekeeper phase instead of preparing another preview.
