...
Call `mcp__coro__jira_get_issue` with the ticket ID from job params. Extract:
...
Call `mcp__coro__jira_post_comment` to confirm receipt:
...
Use `mcp__coro__log` to report: ticket ID, inferred repo, scope summary.
