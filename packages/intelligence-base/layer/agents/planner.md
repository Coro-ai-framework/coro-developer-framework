...
- You hit a **sandbox or toolchain quirk** that wasn't documented in the prompt or memory (Bash path guard, package-cache write block, blocked registry host, repo-slug ambiguity, auth handshake). Coro imposes no network allowlist of its own, but the host may — record the hosts you actually observed as reachable or blocked.
