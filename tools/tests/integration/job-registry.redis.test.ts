...
import { resolveA5aiRoot } from './repo-root'
...
describe.skipIf(skipRedis)('JobRegistry (Redis integration)', () => {
  let redis: Redis
  ...
  beforeEach(async () => {
    ...
    registry = new JobRegistry(redis, a5aiRoot, noopLogger)
  })
  ...
  describe('createJob + getJob', () => {
    ...
    it('returns null for unknown job id', async () => {
      expect(await registry.getJob('no-such-job')).toBeNull()
    })
  })

  describe('indexes and listings', () => {
    it('adds job id to global and type sets', async () => {
      const j1 = await registry.createJob({
        type: 'migration',
        params: { serviceName: 'm1', repoSlug: 'r1' },
      })
      const j2 = await registry.createJob({
        type: 'feature',
  ...
  describe('updateJob', () => {
    ...
    it('throws when job does not exist', async () => {
      await expect(registry.updateJob('missing-id', { status: 'complete' })).rejects.toThrow(
        'Job not found',
      )
    })
  })
  ...
  describe('deleteJob', () => {
    it('removes job document, log, and set memberships', async () => {
      ...
      await registry.appendLog(job.id, 'line1')

      await registry.deleteJob(job.id)

      expect(await registry.getJob(job.id)).toBeNull()
    ...
    it('is idempotent when job is already gone', async () => {
      await expect(registry.deleteJob('never-created')).resolves.toBeUndefined()
    })
  })
  ...
  describe('PR and Jira mappings', () => {
    it('mapPrToJob and getJobByPr roundtrip', async () => {
      ...
      await registry.mapPrToJob(777, job.id)

      const found = await registry.getJobByPr(777)
    ...
    it('mapJiraTicketToJob and getJobByJiraTicket roundtrip', async () => {
      ...
      await registry.mapJiraTicketToJob('XYZ-1', job.id)

      const found = await registry.getJobByJiraTicket('XYZ-1')
      expect(found!.id).toBe(job.id)
    })
    ...
    it('mapRepoToJob adds job id to repo set', async () => {
      ...
      await registry.mapRepoToJob('another-repo', job.id)
  ...
  describe('PR mappings on Job', () => {
    it('addPrMapping appends and registers pr key', async () => {
      ...
      const updated = await registry.addPrMapping(job.id, {
        prId: 1001,
        feature: 'feat-a',
        repoSlug: 'apm',
    ...
    it('markPrMerged sets mergedAt on the matching PR', async () => {
      ...
      const merged = await registry.markPrMerged(job.id, 2002, '2026-04-05T00:00:00Z')
    ...
    it('throws markPrMerged when job is missing', async () => {
      await expect(registry.markPrMerged('missing', 1, '2026-01-01')).rejects.toThrow('Job not found')
    })
  })
  ...
  describe('appendLog / getLog / logLength', () => {
    it('stores chronological log lines', async () => {
      ...
      await registry.appendLog(job.id, 'first')
      await registry.appendLog(job.id, 'second')

      expect(await registry.logLength(job.id)).toBe(2)
      const lines = await registry.getLog(job.id)
    ...
    it('getLog supports range slicing', async () => {
      ...
      await registry.appendLog(job.id, 'a')
      await registry.appendLog(job.id, 'b')
      await registry.appendLog(job.id, 'c')

      const slice = await registry.getLog(job.id, 0, 1)
