      ...
      if (code !== 0 && !token) {
        ...
        res.status(500).json({
          ...
          message: scriptPtyFail
            ? 'The runner has no controlling terminal, so the Claude Code CLI cannot allocate a PTY for the token flow. Start the runner from a terminal (e.g. `a5 runner start` in Terminal.app) and retry, or paste a token generated elsewhere.'
            : undefined,
          stderr: stderr.trim().slice(0, 2000),
          authUrl,
        })
