...
After writing the file, call `mcp__a5__post_artifact` using the workflow-defined title/kind/path. If the workflow does not specify them, use `kind: "evaluation-md"`.

### 5. Manage the work-item loop

1. Call `mcp__a5__update_work_item` to set the current work item's status:
   - `complete` if all critical tests pass
   - Keep `in-progress` if looping back for fixes

2. If looping back, increment the loop count:
   - Call `mcp__a5__update_work_item` with `incrementLoop: true`
   - Call `mcp__a5__get_work_items` to check the loop count
...
3. If the current work item is complete, check for more work items:
   - Call `mcp__a5__get_work_items` to see remaining work items
   - If pending work items remain: call `mcp__a5__request_new_session` (fresh context for the next work item), then call `mcp__a5__goto_phase("coding")`
   - If all work items are complete: do nothing — the runner auto-advances to the next phase

### 6. Log progress

Use `mcp__a5__log` to report: evaluation decision, work-item status, loop count, any memory updates made.
